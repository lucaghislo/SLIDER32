function slider32(folder_name)
clearvars -except start stop iFolder folder_name
% clc
...
if ~isempty(pedestals_mean_values)
    ...
    if (calculate_fit == 1)
        [pedestalFitData,pedestals_mean_values_wo_fit] = pedestal_fit(pedestalAllData,pedestal_data_for_ch_mean, channels_to_not_consider, plot_PEDESTALS, folder_name);
    end
    if exist('transfer_function_mean_values','var')
        disp('plot ENC - START');
        ENCs = ENC(slopes_and_interceptes,pedestals_mean_values, plot_ENC, channels_to_not_consider, folder_name);
        disp('plot ENC - END');
        if calculate_mean_pedestals==1
            disp('plot ENC without mean - START');
            ENCs_wo_mean = ENC_Pedestal_wo_mean(slopes_and_interceptes,pedestals_mean_values_wo_mean, plot_ENC, channels_to_not_consider, folder_name);
            disp('plot ENC without mean - END');
        end
        if calculate_fit == 1
            disp('plot ENC without fit - START');
            ENCs_wo_fit = ENC_Pedestal_wo_fit(slopes_and_interceptes,pedestals_mean_values_wo_fit, plot_ENC, channels_to_not_consider, folder_name);
            disp('plot ENC without mean - END');
        end
    end
