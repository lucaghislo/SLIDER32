function [ENCs_wo_fit] = ENC_Pedestal_wo_fit(slopes_and_interceptes,pedestals_mean_values_wo_fit, plot_ENC, channels_to_not_consider, folder_name)
...
channels2 = unique(pedestals_mean_values_wo_fit(:,1));
peaking_times2 = unique(pedestals_mean_values_wo_fit(:,2));
...
if(all(channels1 == channels2) && all(peaking_times1 == peaking_times2))
    ...
    %% CALCOLO ENC
    ENCs_wo_fit = zeros(length_channels*length_peaking_times,6);
    for ch = channels'
        for pt = peaking_times'
            rms_mean = pedestals_mean_values_wo_fit(pedestals_mean_values_wo_fit(:,1)==ch & pedestals_mean_values_wo_fit(:,2)==pt,4);
            rms_mean_wo_out = pedestals_mean_values_wo_fit(pedestals_mean_values_wo_fit(:,1)==ch & pedestals_mean_values_wo_fit(:,2)==pt,8);
    ...
    for ch_n = channels_to_not_consider
        ENCs_wo_fit(ch_n*8+1: ch_n*8+8,3:6)=nans;
    end
    ...
    if(plot_ENC(6)==1)
        ...
        if ~exist([folder_name 'analysis_matlab/ENC'],'dir' )
            ...
            mkdir([folder_name 'analysis_matlab/ENC/wo_fit']);
            mkdir([folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH']);
        else
            if ~exist([folder_name 'analysis_matlab/ENC/wo_fit/'],'dir' )
                mkdir([folder_name 'analysis_matlab/ENC/wo_fit']);
                mkdir([folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH']);
            else
                if~exist([folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH'],'dir' )
                    mkdir([folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH']);
                end
            end
        end
        ...
        idx_pt = 1;
        for pt = peaking_times'
            data = ENCs_wo_fit(ENCs_wo_fit(:,2) == pt,3);
        ...
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (linear-mean) - Lines.'];
        ...
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (linear-mean) - Bar Plot.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (linear-mean) - Heat Map.'];
        % new figure width
        ...
        idx_pt = 1;
        for pt = peaking_times'
            data = ENCs_wo_fit(ENCs_wo_fit(:,2) == pt,4);
            data_to_plot(:,idx_pt) = data;
            plot(channels,data,'Marker','*')
            idx_pt = idx_pt + 1;
        end
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (linear-mean_wo_out) - Lines.'];
        save_image(svg_to_save, 'pdf', f)
        % annotation('textbox', [.7 .6 .1 .1], 'String','4 keV','FitBoxToText','on', 'Color','red')
        close
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (linear-mean_wo_out) - Bar Plot.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (linear-mean_wo_out) - Heat Map.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        idx_pt = 1;
        for pt = peaking_times'
            data = ENCs_wo_fit(ENCs_wo_fit(:,2) == pt,5);
            data_to_plot(:,idx_pt) = data;
            plot(channels,data,'Marker','*')
            idx_pt = idx_pt + 1;
        end
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (cubic-mean) - Lines.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (cubic-mean) - Bar Plot.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        ylabel('Peaking times');
        f.WindowState = 'fullscreen';
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (cubic-mean) - Heat Map.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        idx_pt = 1;
        for pt = peaking_times'
            data = ENCs_wo_fit(ENCs_wo_fit(:,2) == pt,6);
            data_to_plot(:,idx_pt) = data;
            plot(channels,data,'Marker','*')
            idx_pt = idx_pt + 1;
        end
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (cubic-mean_wo_out) - Lines.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (cubic-mean_wo_out) - Bar Plot.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        ylabel('Peaking times');
        f.WindowState = 'fullscreen';
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_CH/ENC (cubic-mean_wo_out) - Heat Map.'];
        save_image(svg_to_save, 'pdf', f)
        close
    ...
    if(plot_ENC(5)==1)
        ...
        if ~exist([folder_name 'analysis_matlab/ENC'],'dir' )
            mkdir([folder_name 'analysis_matlab/ENC']);
            mkdir([folder_name 'analysis_matlab/ENC/wo_fit']);
            mkdir([folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT']);
        else
            if ~exist([folder_name 'analysis_matlab/ENC/wo_fit/'],'dir' )
                mkdir([folder_name 'analysis_matlab/ENC/wo_fit']);
                mkdir([folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT']);
            else
                if~exist([folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT'],'dir' )
                    mkdir([folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT']);
                end
            end
        end
        ...
        grid on
        for ch = channels'
            data = ENCs_wo_fit(ENCs_wo_fit(:,1) == ch,3);
        ...
        Y_Lim = def_ax.YLim(2);
        def_ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (linear-mean) - Lines.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (linear-mean) - Bar Plot.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        ylabel('Channels');
        f.WindowState = 'maximized';
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (linear-mean) - Heat Map.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        grid on
        for ch = channels'
            data = ENCs_wo_fit(ENCs_wo_fit(:,1) == ch,4);
            data_to_plot(ch + 1,:) = data';
            plot(peaking_times,data,'Marker','*')
        end
        yline(4,'-.','Color','r','LineWidth',1.4);
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (linear-mean_wo_out) - Lines.'];
        save_image(svg_to_save, 'pdf', f)
        % annotation('textbox', [.7 .6 .1 .1], 'String','4 keV','FitBoxToText','on', 'Color','red')
        close
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (linear-mean_wo_out) - Bar Plot.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (linear-mean_wo_out) - Heat Map.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        grid on
        for ch = channels'
            data = ENCs_wo_fit(ENCs_wo_fit(:,1) == ch,5);
            data_to_plot(ch + 1,:) = data';
            plot(peaking_times,data,'Marker','*')
        end
        yline(4,'-.','Color','r','LineWidth',1.4);
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (cubic-mean) - Lines.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (cubic-mean) - Bar Plot.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        ylabel('Channels');
        f.WindowState = 'maximized';
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (cubic-mean) - Heat Map.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        grid on
        for ch = channels'
            data = ENCs_wo_fit(ENCs_wo_fit(:,1) == ch,6);
            data_to_plot(ch + 1,:) = data';
            plot(peaking_times,data,'Marker','*')
        end
        yline(4,'-.','Color','r','LineWidth',1.4);
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (cubic-mean_wo_out) - Lines.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        ax = f.CurrentAxes;
        ax.YLim = [0,Y_Lim];
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (cubic-mean_wo_out) - Bar Plot.'];
        save_image(svg_to_save, 'pdf', f)
        close
        ...
        svg_to_save = [folder_name 'analysis_matlab/ENC/wo_fit/Plot_PT/ENC (cubic-mean_wo_out) - Heat Map.'];
        save_image(svg_to_save, 'pdf', f)
        close
    ...
    if ~exist([folder_name 'analysis_matlab/ENC/wo_fit/ENC_wo_fit.dat'],'file') % Creo il file solo se non esiste già
        fileID = fopen([folder_name 'analysis_matlab/ENC/wo_fit/ENC_wo_fit.dat'],'w');
        ...
        fprintf(fileID,format,ENCs_wo_fit');
        fclose(fileID);
    end
end
end
